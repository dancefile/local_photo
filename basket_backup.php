...
foreach ($_SESSION['name'] as $i => $value) {
	$k++;
 echo '<div class="photo" id="foto'.$i.'"><img class="img" title="'.$_SESSION['name'][$i].'.jpg" src="/ps/'.$_SESSION['name'][$i].'.jpg" hspace="5" vspace="10">'.
 digital.'&nbsp;<input type="checkbox"  id="cd'.$i.'" class="count_foto cd" value="" ';
if ($_SESSION['cd'][$i]) echo 'checked';
echo ' onClick="price();"><span id="cd'.$i.'b">&nbsp;<img src="img/load2_emtry.gif"/>&nbsp;</span><br><br>
...
'.comment.'&nbsp;<input name="comm['.$i.']" type="text" size="8" id="comm'.$i.'" value="'.$_SESSION['comm'][$i].'" class="count_foto i_kom"><span id="comm'.$i.'b">&nbsp;<img src="img/load2_emtry.gif"/>&nbsp;</span>';
 echo '<br>'.price.'<span id="price'.$i.'"  class="">0</span><br>
<span id="del'.$i.'" class="link del">'.deletephoto.'</span></div>';

};
?><br>
<? echo total.': <span id="total"></span> '.curence.'<br>'.photos.'<span id="count"></span><br>';
 if (isset($_SESSION['login'])) {?> <br clear="all"><input id="paid" type="checkbox" checked> <? echo Paid; }; ?>
<br clear="all"><br>
E-mail: <input type="text" id="mail" value="<?=$_SESSION['mail']; ?>" class="count_foto i_mail"><span id="mailb">&nbsp;<img src="img/load2_emtry.gif"/>&nbsp;</span><br><br>
<button id="send" class="buttom"><? echo Makeorder; ?></button>
<br><br><br>
<span class="link" id="del"><? echo CleanCart; ?></span></span>
<?
} else {if (isset($echo1)) {echo $echo1;} else {echo '<br><br>'.cartempty;};};
?>
 <div class="modalWindow"><center id="center"><table height="100%"><tr><td width="100px"><img src="img/left.png" class="g_but" id="left_b"/></td><td align="center">
 	<div class="crop" id="divimg"><img src="img/nopictures.png" id="img_l"/><img src="img/loading.gif" id="img_z"/><img src="img/loading.gif" id="img"/></div><br><span id="fancybox-title-over"></span></td><td width="100px">
 	<img src="img/right.png" class="g_but" id="right_b"/></td></tr></table></center></div>
